# SQL Database Assistant with Safe Execution
...
1. **Clone the repository:**
   ```bash
   git clone https://github.com/erhatechnologiesai/sql-database-agent.git
...
|---|---|---|
| `POST` | `/query-database` | Convert English question to safe SQL, execute, and return formatted results |

### Example Request

```bash
curl -X POST http://127.0.0.1:8000/query-database -H "Content-Type: application/json" -d '{"question": "Show top 5 customers by total order spend"}'
```
...
```bash
python -m unittest tests/test_sql_agent.py
```
